    ...
    <div class="glass-card rounded-2xl border border-slate-800 bg-slate-900/80 p-6 sm:p-8 shadow-xl backdrop-blur-md">
        ...
        <form method="POST" action="{{ route('login') }}" class="mt-7 space-y-5">
            ...
            <div>
                ...
                <!-- Diubah dari text-black menjadi text-white -->
                <input id="email" class="w-full rounded-xl bg-slate-950/80 border border-slate-700/60 px-4 py-2.5 text-sm text-white placeholder-slate-600 focus:border-indigo-500 focus:outline-none transition" type="email" name="email" value="{{ old('email') }}" required autofocus autocomplete="username">
            ...
            <div>
                ...
                <!-- Bungkus dengan relative agar ikon mata bisa diposisikan di dalam input -->
                <div class="relative">
                    <!-- Diubah dari text-black menjadi text-white serta ditambahkan padding-right (pr-10) agar teks tidak tertutup ikon -->
                    <input id="password" class="w-full rounded-xl bg-slate-950/80 border border-slate-700/60 pl-4 pr-10 py-2.5 text-sm text-white placeholder-slate-600 focus:border-indigo-500 focus:outline-none transition" type="password" name="password" required autocomplete="current-password">
                    
                    <!-- Tombol / Icon Show-Hide Password -->
                    <button type="button" onclick="togglePassword()" class="absolute inset-y-0 right-0 flex items-center pr-3 text-slate-400 hover:text-slate-200 transition focus:outline-none">
