<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="glass-card rounded-2xl p-6 lg:p-8 relative overflow-hidden bg-gradient-to-r from-slate-900 via-slate-800/90 to-blue-950 dark:from-slate-900 dark:via-slate-800/90 dark:to-blue-950 border border-slate-200 dark:border-slate-700/50 shadow-sm">
            <div class="relative z-10 max-w-2xl">
                <h1 class="text-2xl lg:text-3xl font-bold text-slate-900 dark:text-white tracking-tight">
                    Ringkasan Keuangan & Stok
                </h1>
                <p class="mt-2 text-slate-600 dark:text-slate-400 text-sm lg:text-base leading-relaxed">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            <!-- Pemasukan Bulan Ini -->
            <div class="glass-card rounded-2xl p-5 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 shadow-sm">
                <div class="flex items-center justify-between">
                    <span class="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Pemasukan (Bulan Ini)</span>
            ...
            <div class="glass-card rounded-2xl p-5 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 shadow-sm">
                <div class="flex items-center justify-between">
                    <span class="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Pengeluaran (Bulan Ini)</span>
            ...
            <div class="glass-card rounded-2xl p-5 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 shadow-sm">
                <div class="flex items-center justify-between">
                    <span class="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Saldo Kas & Bank</span>
                ...
                <div class="mt-4">
                    <div class="text-2xl font-bold {{ $cashBalance >= 0 ? 'text-slate-900 dark:text-white' : 'text-rose-600 dark:text-rose-400' }}">
            ...
            <div class="glass-card rounded-2xl p-5 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 shadow-sm">
                <div class="flex items-center justify-between">
                    <span class="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Nilai Inventaris</span>
        ...
        <div class="glass-card rounded-2xl p-6 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 shadow-sm">
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h3 class="text-lg font-bold text-slate-900 dark:text-white">Tren Keuangan</h3>
                    <p class="text-xs text-slate-500 dark:text-slate-400">Perbandingan pemasukan dan pengeluaran selama 7 bulan terakhir</p>
                </div>
            ...
            <div class="h-64 flex items-end justify-between gap-2 lg:gap-6 pt-8 pb-2 px-2 border-b border-slate-200 dark:border-slate-800">
                @foreach($chart as $item)
                    ...
                    <div class="flex-1 flex flex-col items-center h-full justify-end group relative">
                        <!-- Tooltip Hover -->
                        <div class="absolute -top-12 hidden group-hover:flex flex-col items-center bg-slate-900 dark:bg-slate-950 text-white text-[10px] py-1.5 px-3 rounded-lg border border-slate-700 shadow-xl z-20 whitespace-nowrap pointer-events-none">
                        ...
                        <!-- Label Bulan -->
                        <span class="mt-3 text-[11px] font-medium text-slate-500 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-white transition">
                            {{ $item['label'] }}
                        </span>
                    </div>
                @endforeach
        ...
        <div class="glass-card rounded-2xl p-6 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 shadow-sm">
            <div class="flex items-center justify-between mb-4">
                <div>
                    ...
                    <p class="text-xs text-slate-500 dark:text-slate-400">Daftar aktivitas keuangan terbaru</p>
                </div>
                <div class="flex gap-2">
                    <a href="{{ route('income.index') }}" class="text-xs font-medium text-emerald-600 dark:text-emerald-400 hover:underline">Pemasukan &rarr;</a>
                    <span class="text-slate-300 dark:text-slate-600">|</span>
                    <a href="{{ route('expense.index') }}" class="text-xs font-medium text-rose-600 dark:text-rose-400 hover:underline">Pengeluaran &rarr;</a>
                </div>
            </div>

            <div class="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-800/80">
                <table class="w-full text-left text-sm text-slate-700 dark:text-slate-300">
                    <thead class="bg-slate-100 dark:bg-slate-900 text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider border-b border-slate-200 dark:border-slate-800">
                        <tr>
                    ...
                    <tbody class="divide-y divide-slate-200 dark:divide-slate-800/60 bg-white dark:bg-slate-900/20">
                        @forelse($latestTransactions as $tx)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition">
                                ...
                                <td class="px-5 py-4 text-right font-bold whitespace-nowrap {{ $tx['type'] === 'masuk' ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400' }}">
