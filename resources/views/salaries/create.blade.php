<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto space-y-6">
        <div>
            <h1 class="text-2xl font-extrabold text-white tracking-tight">Input Gaji Karyawan</h1>
            <!-- PENGAMAN: Mencegah error jika tanggal kosong, menggunakan attendance_date -->
            <p class="mt-1 text-slate-400 text-sm">
                Pembayaran untuk: <span class="text-indigo-400 font-semibold">{{ $attendance->person->name ?? '-' }}</span> 
                (Absen Tgl: {{ optional($attendance->attendance_date)->format('d/m/Y') ?? '-' }})
            </p>
        </div>

        <div class="glass-card rounded-2xl border border-slate-800 bg-slate-900/60 p-6 sm:p-8">
            <form action="{{ route('salaries.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6" id="salaryForm">
                ...
                <!-- Tanggal Gaji -->
                <div>
                    <label class="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Tanggal Pembayaran Gaji</label>
                    <input type="date" name="salary_date" value="{{ date('Y-m-d') }}" required class="w-full rounded-xl bg-slate-950/80 border border-slate-700/60 px-4 py-2.5 text-sm text-white focus:border-indigo-500 focus:outline-none transition">
                </div>

                <!-- Nominal Gaji -->
                <div>
                    <label class="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Nominal Gaji (Rp)</label>
                    <input type="number" name="amount" placeholder="Contoh: 70000" required class="w-full rounded-xl bg-slate-950/80 border border-slate-700/60 px-4 py-2.5 text-sm text-white placeholder-slate-600 focus:border-indigo-500 focus:outline-none transition">
                ...
                <div>
                    <label class="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Tanda Tangan Penerima</label>
                    <div class="border border-slate-700/60 rounded-xl bg-slate-950/80 p-2 text-center">
                        <canvas id="sig-canvas" width="400" height="150" class="w-full bg-slate-900 rounded-lg cursor-crosshair border border-slate-800"></canvas>
                        <div class="mt-2 flex justify-between items-center px-2">
                            <button type="button" id="sig-clearBtn" class="text-xs text-rose-400 hover:underline">Hapus / Ulangi Tanda Tangan</button>
                            <span class="text-[10px] text-slate-500">Tanda tangani di atas kotak</span>
                        </div>
                    </div>
                ...
                <div>
                    ...
                    <textarea name="notes" rows="2" placeholder="Catatan opsional..." class="w-full rounded-xl bg-slate-950/80 border border-slate-700/60 px-4 py-2.5 text-sm text-white placeholder-slate-600 focus:border-indigo-500 focus:outline-none transition"></textarea>
                </div>

                <!-- Tombol Aksi -->
                <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-800">
                    <a href="{{ route('salaries.index') }}" class="px-5 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 font-semibold text-sm transition">Batal</a>
