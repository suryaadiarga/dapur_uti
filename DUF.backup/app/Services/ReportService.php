...
class ReportService
{
    ...
    public function build(array $filters, ?User $user = null): array
    {
        $type = array_key_exists($filters['type'] ?? '', self::TYPES) ? $filters['type'] : 'income';
        [$dateFrom, $dateTo, $periodLabel] = $this->dateRange($filters);

        return match ($type) {
            'expense' => $this->expense($dateFrom, $dateTo, $periodLabel, $user),
            'cash' => $this->cash($dateFrom, $dateTo, $periodLabel, $user),
            'receipts' => $this->receipts($dateFrom, $dateTo, $periodLabel, $user),
            'inventory' => $this->inventory($dateFrom, $dateTo, $periodLabel, $user),
            default => $this->income($dateFrom, $dateTo, $periodLabel, $user),
        };
    }
    ...
    private function income(string $from, string $to, string $period, ?User $user): array
    {
    ...
    private function expense(string $from, string $to, string $period, ?User $user): array
    {
    ...
    private function cash(string $from, string $to, string $period, ?User $user): array
    {
        $items = $this->cashService->mutations($from, $to, $user);
    ...
    private function receipts(string $from, string $to, string $period, ?User $user): array
    {
    ...
    private function inventory(string $from, string $to, string $period, ?User $user): array
    {
