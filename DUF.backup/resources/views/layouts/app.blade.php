...
<div x-data="{ sidebarOpen: false }" class="min-h-screen">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-40 flex w-72 flex-col bg-emerald-950 text-white transition-transform duration-200 lg:translate-x-0">
        <div class="flex h-20 items-center gap-3 border-b border-white/10 px-6">
            ...
            <div>
                ...
                <div class="text-xs text-emerald-200">Finance</div>
            </div>
        </div>
        ...
        <nav class="flex-1 space-y-1 overflow-y-auto p-4 text-sm">
            @php
                ...
                if (auth()->user()->isAdmin()) {
                    $menus[] = ['users.*', 'users.index', 'Manajemen User', 'U'];
                    $menus[] = ['activity-logs.*', 'activity-logs.index', 'Activity Log', 'A'];
                    $menus[] = ['settings.*', 'settings.edit', 'Pengaturan', 'P'];
                }
            @endphp
            @foreach($menus as [$pattern, $route, $label, $icon])
                <a href="{{ route($route) }}" @click="sidebarOpen = false"
                   class="flex items-center gap-3 rounded-xl px-4 py-3 transition {{ request()->routeIs($pattern) ? 'bg-white text-emerald-950 shadow-sm' : 'text-emerald-50 hover:bg-white/10' }}">
        ...
        <div class="border-t border-white/10 p-4">
            <div class="mb-3 px-4 text-xs text-emerald-200">Login sebagai</div>
            <div class="flex items-center justify-between gap-3 px-4">
                <div class="min-w-0">
                    <div class="truncate text-sm font-medium">{{ auth()->user()->name }}</div>
                    <div class="truncate text-xs text-emerald-200">{{ auth()->user()->username }} · {{ ucfirst(auth()->user()->role) }}</div>
                </div>
    ...
    <div class="lg:pl-72">
        <header class="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-stone-200 bg-white/90 px-4 backdrop-blur sm:px-6 lg:px-8">
            ...
            <div class="ml-auto text-sm text-stone-500">{{ now()->translatedFormat('l, d F Y') }}</div>
        </header>
