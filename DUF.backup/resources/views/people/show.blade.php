            ...
            <dl class="grid gap-5 sm:grid-cols-2">
                @if(auth()->user()->isAdmin())
                    <div class="sm:col-span-2">
                        <dt class="text-xs uppercase text-stone-500">Account Terkait</dt>
                        <dd class="mt-1">
                            @if($person->user)
                                <div class="font-medium">{{ $person->user->username }} - {{ $person->user->name }}</div>
                                <div class="text-sm text-stone-500">User ID {{ $person->user_id }} · {{ $person->user->email }}</div>
                            @else
