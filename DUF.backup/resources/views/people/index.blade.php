            ...
            @if(auth()->user()->isAdmin())
                <div>
                    ...
                    <select name="user_id" class="form-control mt-1">
                        ...
                        <option value="unassigned" @selected(request('user_id') === 'unassigned')>Tanpa account</option>
                        @foreach($users as $account)
                            <option value="{{ $account->id }}" @selected((string) request('user_id') === (string) $account->id)>
    ...
    <div class="panel">
        <div class="table-wrap">
            <table class="data-table">
                <thead><tr><th>Nama</th><th>Nomor HP</th><th>Peran</th>@if(auth()->user()->isAdmin())<th>Pemilik Data</th>@endif<th>Catatan</th><th class="text-right">Aksi</th></tr></thead>
                <tbody>
                @forelse($people as $person)
                    <tr>
                        ...
                        @if(auth()->user()->isAdmin())
                            <td>
                                @if($person->user)
                                    <div class="font-medium">{{ $person->user->username }}</div>
                                    <div class="text-xs text-stone-500">ID {{ $person->user_id }} · {{ $person->user->name }}</div>
                                @else
                                    <span class="text-stone-500">Tanpa account</span>
                                @endif
                            </td>
                        @endif
                ...
                @empty
                    <tr><td colspan="{{ auth()->user()->isAdmin() ? 6 : 5 }}" class="py-10 text-center text-stone-500">Belum ada data orang.</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
