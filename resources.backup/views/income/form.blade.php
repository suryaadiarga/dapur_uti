<x-app-layout>
    ...
    <div class="mb-6"><a href="{{ route('income.index') }}" class="text-sm font-medium text-emerald-700">← Kembali</a><h1 class="page-title mt-3">{{ $transaction->exists ? 'Edit Uang Masuk' : 'Tambah Uang Masuk' }}</h1></div>
    <form method="POST" enctype="multipart/form-data" action="{{ $transaction->exists ? route('income.update', $transaction) : route('income.store') }}" class="panel panel-body max-w-4xl">
        ...
        <div class="grid gap-5 sm:grid-cols-2">
            ...
            <div><label class="form-label">Metode pembayaran <span class="text-red-500">*</span></label><select name="payment_method" class="form-control mt-1" required>@foreach(\App\Models\IncomeTransaction::PAYMENT_METHODS as $value => $label)<option value="{{ $value }}" @selected(old('payment_method', $transaction->payment_method ?? 'tunai') === $value)>{{ $label }}</option>@endforeach</select></div>
            <div><label class="form-label">Bukti/foto</label><input type="file" name="proof" accept=".jpg,.jpeg,.png,.webp" class="form-control mt-1"><p class="mt-1 text-xs text-stone-500">JPG, PNG, WEBP. Maksimal 2MB.</p></div>
            ...
            @if($transaction->proof_path)<div class="sm:col-span-2"><div class="text-xs text-stone-500">Bukti saat ini</div><img src="{{ Storage::url($transaction->proof_path) }}" class="mt-2 max-h-52 rounded-xl border object-contain" alt="Bukti"></div>@endif
        </div>
        <div class="mt-6 flex gap-3"><button class="btn-primary">Simpan</button><a href="{{ route('income.index') }}" class="btn-secondary">Batal</a></div>
    </form>
</x-app-layout>
