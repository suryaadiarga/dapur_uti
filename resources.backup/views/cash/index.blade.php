<x-app-layout>
    ...
    <div class="mb-6 grid gap-4 sm:grid-cols-3">
        <div class="panel panel-body"><div class="text-sm text-sky-500">Total uang masuk</div><div class="mt-2 text-2xl font-semibold text-blue-700">Rp {{ number_format($totalIncome, 0, ',', '.') }}</div></div>
        ...
        <div class="panel panel-body"><div class="text-sm text-sky-500">Saldo akhir</div><div class="mt-2 text-2xl font-semibold {{ $balance < 0 ? 'text-red-700' : 'text-sky-900' }}">Rp {{ number_format($balance, 0, ',', '.') }}</div></div>
    </div>
    <div class="panel">
            ...
            <table class="data-table">
                ...
                @forelse($mutations as $item)
                    <tr>
                        ...
                        <td>{{ $item['person'] }}</td>
                        <td class="whitespace-nowrap text-blue-700">{{ $item['debit'] ? 'Rp '.number_format($item['debit'], 0, ',', '.') : '-' }}</td>
